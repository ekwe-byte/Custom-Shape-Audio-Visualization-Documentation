...
`Absolute Position`:
+ Checked: The algorithm uses the **shape centroid** (the actual average position of the child points) as the origin for rendering.
+ Unchecked: The algorithm captures your shape, calculates its rotation, and then offsets the entire shape to the **Script Layer's position** (*center*).
